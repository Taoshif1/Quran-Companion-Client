...
export default function SurahsPage() {
  ...
  return <div className="surahs-page page-shell"><header className="page-intro"><p className="eyebrow">The Noble Quran · 114 chapters</p><h1>Choose a Surah</h1><p>Browse by name, number, meaning, or place of revelation.</p></header><div className="surah-controls"><label className="search-field"><Search/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search Surahs" aria-label="Search Surahs"/></label><div className="segmented" aria-label="Revelation place">{[["all","All"],["makkah","Meccan"],["madinah","Medinan"]].map(([value,label]) => <button key={value} className={place === value ? "active" : ""} onClick={() => setPlace(value)}>{label}</button>)}</div></div>{query.isLoading && !chapters.length ? <LoadingState/> : query.isError && !chapters.length ? <EmptyState title="Local Quran data could not be loaded." message="Please reload the application." offline={!navigator.onLine}/> : <div className="surah-list">{filtered.map((chapter) => <SurahRow key={chapter.id} chapter={chapter} downloaded={downloaded.has(chapter.id)}/>)}</div>}</div>;
}
