...
export default function SurahsPage() {
  ...
  const filtered = useMemo(() => chapters.filter((chapter) => {
    ...
    const matchesSearch = !needle || String(chapter.id) === needle || [chapter.nameSimple, chapter.transliteratedName, chapter.translatedName, chapter.nameArabic].some((name) => normalizeSearch(name).includes(needle));
...
function normalizeSearch(value = "") {
  return value.toLowerCase().trim().replace(/^al[-\s]/, "").replace(/[^\p{L}\p{N}]+/gu, "");
}
